...
**Production Requirements**:
- Every activity is idempotent under Temporal retries; writes use deterministic IDs/upserts.
...
- **Structured logging schema**: consistent fields (`user_id`, `upload_id`, `workflow_id`, `step_name`, `attempt`, `vendor`, `cost_usd`, `latency_ms`).
