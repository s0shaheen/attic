...
4. Upload ZIP
   └── Drag-and-drop upload, file validation
...
**Requirements**:
- Accept ZIP file upload (max 500MB)
...
**Acceptance Criteria**:
- [ ] User can drag-and-drop or click to upload ZIP
...
**Acceptance Criteria**:
- [ ] Progress endpoint returns current state for each step
- [ ] User sees step-by-step progress in UI
- [ ] User can leave page; processing continues
- [ ] User receives notification (email or SMS) when done
...
**Acceptance Criteria**:
- [ ] Progress updates in real-time (polling or websocket)
- [ ] Estimated time is reasonably accurate
- [ ] Email notification sent on completion
- [ ] SMS notification works if opted in
...
```sql
-- Job queue table (poor man's queue)
CREATE TABLE job_queue (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    queue_name VARCHAR(50) NOT NULL,  -- 'apify_enrich', 'vision_analysis', etc.
...
CREATE INDEX idx_job_queue_fetch ON job_queue(queue_name, status, scheduled_for, priority)
...
UPDATE job_queue
SET status = 'processing', locked_by = $worker_id, locked_at = NOW(), attempts = attempts + 1
WHERE id = (
